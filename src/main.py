...
class MainView:
	...
	def home(current_screen):
		...
		# Logo (brand) Skyline
		logo_container = tk.Canvas(home_screen, bg=MainView.BG_03, highlightthickness=0)
		logo_container.grid(
			row=0,
			column=0,
			ipadx=640,
		...
		logo = tk.Label(logo_container, text='SKYLINE', justify='left')
		logo.grid(row=0, column=0)

		# Initialize window
		window = tk.Canvas(home_screen, height=MainView.HEIGHT, width=MainView.WIDTH)
		...
		search_btn = tk.Button(window, image=search_img, bg=MainView.BG_03, command=lambda: TicketView.search_ticket(home_screen, search_entry.get()))
		...
		buy_ticket_btn = ttk.Button(window, image=buy_ticket_img, command=lambda: TicketView.buy_ticket(home_screen))
		...
		upgrade_btn = ttk.Button(window, image=upgrade_img, command=lambda: TicketView.upgrade_ticket(home_screen))
		...
		explore_btn = ttk.Button(window, image=explore_img, command=lambda: MapView.show_map(home_screen))
		explore_btn.grid(row=5, column=1)

		# Transport button
		transport_img = tk.PhotoImage(file=PATH_IMG+'\\transport_btn.png')
		transport_btn = ttk.Button(window, image=transport_img, command=lambda: TransportView.show_transports(home_screen))
